...
class RamificacionAcotacioncostes(Queue):
    ...
    def append(self, item):
        self.miLista.append(item)
        # print(self.list[0].path_cost)
        # ordenamos los nodos segun su valor interno de menor a valor
        # self.ascendingOrder()
        #print self.list

    def extend(self, items):
        self.iteraciones += 1
        print "// " + str(self.iteraciones) + "ª iteracion //"
        print "Le añadimos: " + str(items)
        self.miLista.extend(items)
        self.ordenAscendente()
        print "Lo ordenamos: " + str(self.miLista)

    def pop(self):
        """ Si en primer nodo no es el Objetivo, eliminar esta trayectoria
         de ABIERTA y anadir sus trayectorias descendientes """
        return self.miLista.pop(0)

    def ordenAscendente(self):
        self.miLista.sort(key=lambda nodo: nodo.path_cost, reverse=False)



class RamificacionAcotacionsubestimacion(Queue):
    ...
    def extend(self, items):
        self.iteraciones += 1
        print "// " + str(self.iteraciones) + "ª iteracion //"
        self.nodosExpandidos(items)
        self.miLista.extend(items)
        self.formaAscendente()
        self.ordenLista()
    ...
    def formaAscendente(self):
        self.miLista.sort(key=lambda nodo: nodo.subestimacion, reverse=False)

    def nodosExpandidos(self, nodes):
        print "Le añadimos:",
        for node in nodes:
            print "[" + str(node.state) + ", Coste(" + str(node.path_cost) + "), Estimacion(" + str(node.subestimacion - node.path_cost) + ")]",
        print ""

    def ordenLista(self):
        print "Ya ordenado:",
        for node in self.miLista:
            print "[" + str(node.state) + ", Subestimacion(" + str(node.subestimacion) + ")]",
        print ""
